...
CANNON.Demo = function(options){
    ...
    this.wireframeMaterial = new THREE.MeshLambertMaterial( { color: 0xffffff, wireframe:true } );
    ...
    for (var l = 0; l < body.shapes.length; l++) {
        ...
        case CANNON.Shape.types.BOX:
            ...
            for (var i = 0; i <= divisions; i++) {
                ...
                points.push(new THREE.Vector2(linkSize + Math.sin(a) * chainSize, Math.cos(a) * chainSize));
            }
            ...
            linkEnd0.position.set(0, 0, linkLength);// - linkSize * chainSize);
            linkEnd0.rotation.set(0, -Math.PI / 2, 0);
            link.add(linkEnd0);

            var linkEnd1 = new THREE.Mesh(geometry, this.currentMaterial);
            linkEnd1.position.set(0, 0, -linkLength);// + linkSize * chainSize);
            linkEnd1.rotation.set(0, -Math.PI / 2, Math.PI);
            link.add(linkEnd1);


            // radiusTop, radiusBottom, height, radiusSegments, heightSegments,

            var geometry = new THREE.CylinderBufferGeometry(chainSize, chainSize, linkLength * 2, segments);
            var cylinder = new THREE.Mesh(geometry, this.currentMaterial);
            cylinder.rotation.set(Math.PI / 2, 0, 0);
            cylinder.position.set(linkSize, 0, 0);
            link.add(cylinder);
            var cylinder = new THREE.Mesh(geometry, this.currentMaterial);
            cylinder.rotation.set(Math.PI / 2, 0, 0);
            cylinder.position.set(-linkSize, 0, 0);
