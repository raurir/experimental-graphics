...
describe("geom.polygonSelfIntersecting", () => {
	it("should return false if a polygon is not self intersecting", (done) => {
		...
		expect(geom.polygonSelfIntersecting(pentagon)).toEqual(false);

		const heptagon = poly(7, -1);
		expect(geom.polygonSelfIntersecting(heptagon)).toEqual(false);
	...
	it("should return true if a polygon is self intersecting", (done) => {
		...
		expect(geom.polygonSelfIntersecting(pentagram)).toEqual(true);

		const heptagram = poly(7, -2);
		expect(geom.polygonSelfIntersecting(heptagram)).toEqual(true);
